...
**try** to detect the **Fractal Agentic** plugin and use its domain map + orchestration
guidance. Detection and pins are **best-effort** — they must **never** block the
user’s project work.
...
1. **Read** (as needed for the task):
   - `<root>/AGENTS.md` — especially §0 decision tree and §0.6 delivery runtime
   - `<root>/skills/boss-orchestration/SKILL.md` — non-blocking preflight
   - `<root>/skills/boss-orchestration/references/capability-mode.md` — set mode once
...
5. **Stack defaults** from the plugin map apply unless this project AGENTS.md overrides them.
