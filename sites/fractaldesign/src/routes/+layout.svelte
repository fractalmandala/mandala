<script lang="ts">
	...
	import { theme, isDrawerOpen, toggleDrawer  } from '$lib/utils/globalstores'
...
<div class="{$theme}" inert={$isDrawerOpen ? true : undefined}>
	<header class="row ycenter site-padding xbetween">
		...
		{#if current === '/play/canvas'}
			<button onclick={() => native.toggleSidebar()}>
				Sidebar {native.sidebarCollapsed}
			</button>
			<button class="rightbar" onclick={() => native.toggleRightbar()}>
