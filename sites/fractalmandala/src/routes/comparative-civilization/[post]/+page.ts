...
const modules = import.meta.glob<{ default: Component; metadata: { title?: string, description?: string, tags?: string[], related?: string[] } }>(
	'/src/content/Comparative Civilization/wiki/*.md'
);

export const load: PageLoad = async ({ params }) => {
	const key = `/src/content/Comparative Civilization/wiki/${params.post}.md`;
	const loader = modules[key];
	if (!loader) throw error(404, `Post "${params.post}" not found`);
	const mod = await loader();
	const posts = await listPosts('Comparative Civilization', 'wiki', 'comparative-civilization');
	return {
		content: mod.default,
