...
description: Seven-boss agent orchestration plugin with boss-orchestration runtime, skills/agents/commands inventories, and ship|fix-first|rethink review.
tags: [fractal-agentic, orchestration, bosses, agents, skills, commands]
type: concept
created: 2026-08-04
updated: 2026-08-04
---

# Fractal Agentic System

`packages/fractal-agentic` is a self-described grand orchestration of skills, commands, agents, and bosses under one system that learns, grows, and maintains a wiki. It is the preferred agent process across the whole mandala monorepo — detection is **best-effort and never blocks** project work.
...
When `FRACTAL_WIKI_ROOT` (or fractal wiki config) resolves, prefer `/wiki-query` for prior decisions; after `/orchestrate` completes, soft-capture an episode under `raw/fractal/` (frontmatter **must** include `description`). Missing wiki never blocks product work.

See [[Packages Module]], [[Coding Conventions]], [[Fractal Wiki Vault]].
