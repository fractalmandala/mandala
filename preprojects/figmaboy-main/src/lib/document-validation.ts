import type { DesignNode, PageDocument, Paint, StrokeStyle, ColorStyle, TextStyle, EffectStyle, LayerEffect, ComponentDefinition, Variable, VariableCollection, VariableMode, VariableValue, VariableType } from "$lib/domain";
...
function sanitizeNode(id: string, value: Record<string, unknown>): DesignNode | null {
  ...
  // Variable bindings
  const rawBindings = record(value.boundVariables);
  if (rawBindings) {
    const bindings: Record<string, string> = {};
    for (const [prop, varId] of Object.entries(rawBindings)) {
      if (typeof varId === "string") bindings[prop] = varId;
    }
    if (Object.keys(bindings).length > 0) node.boundVariables = bindings;
  }
  return node;
}

function sanitizeArray<T>(value: unknown, sanitizer: (item: Record<string, unknown>) => T | null): T[] {
  if (!Array.isArray(value)) return [];
  return value.flatMap((item) => {
    const source = record(item);
    return source ? [sanitizer(source)].filter(Boolean) as T[] : [];
  });
}

function sanitizeColorStyle(item: Record<string, unknown>): ColorStyle | null {
  if (typeof item.id !== "string" || typeof item.name !== "string") return null;
  return { ...item, id: item.id, name: item.name, paint: paint(item.paint, { type: "solid", color: "#6366f1", opacity: 1 }) } as ColorStyle;
}

function sanitizeTextStyle(item: Record<string, unknown>): TextStyle | null {
  if (typeof item.id !== "string" || typeof item.name !== "string") return null;
  return {
    ...item, id: item.id, name: item.name,
    fontFamily: string(item.fontFamily, "Inter, sans-serif"),
    fontSize: clamped(item.fontSize, 16, 1, 10000),
    fontWeight: clamped(item.fontWeight, 400, 1, 1000),
    fontStyle: item.fontStyle === "italic" ? "italic" : "normal",
    lineHeight: clamped(item.lineHeight, 1.5, .1, 20),
    letterSpacing: clamped(item.letterSpacing, 0, -10000, 10000),
    textCase: item.textCase === "upper" || item.textCase === "lower" || item.textCase === "title" ? item.textCase : "original",
    textDecoration: item.textDecoration === "underline" || item.textDecoration === "strikethrough" ? item.textDecoration : "none",
    paragraphSpacing: clamped(item.paragraphSpacing, 0, 0, 100000),
  } as TextStyle;
}

function sanitizeEffectStyle(item: Record<string, unknown>): EffectStyle | null {
  if (typeof item.id !== "string" || typeof item.name !== "string") return null;
  const effects: LayerEffect[] = [];
  if (Array.isArray(item.effects)) for (const effect of item.effects) {
    const e = record(effect);
    if (e?.type === "drop-shadow") effects.push({ ...e, type: e.type, color: string(e.color, "#000000"), opacity: clamped(e.opacity, .25, 0, 1), x: finite(e.x, 0), y: finite(e.y, 4), blur: clamped(e.blur, 8, 0, 10000), visible: boolean(e.visible, true) });
    if (e?.type === "layer-blur") effects.push({ ...e, type: e.type, radius: clamped(e.radius, 0, 0, 10000), visible: boolean(e.visible, true) });
  }
  return { ...item, id: item.id, name: item.name, effects } as EffectStyle;
}

const variableTypes = new Set<VariableType>(["color", "number", "string", "boolean"]);

function sanitizeVariableValue(value: unknown): VariableValue {
  const source = record(value);
  if (!source) return { value: "" };
  const raw = source.value;
  if (typeof raw === "string" || typeof raw === "number" || typeof raw === "boolean") return { value: raw };
  return { value: "" };
}

function sanitizeVariableMode(item: Record<string, unknown>): VariableMode | null {
  if (typeof item.id !== "string" || typeof item.name !== "string") return null;
  const valuesSource = record(item.values) ?? {};
  const values: Record<string, VariableValue> = {};
  for (const [varId, val] of Object.entries(valuesSource)) {
    values[varId] = sanitizeVariableValue(val);
  }
  return { id: item.id, name: item.name, values };
}

function sanitizeVariableCollection(item: Record<string, unknown>, validVariableIds: Set<string>): VariableCollection | null {
  if (typeof item.id !== "string" || typeof item.name !== "string") return null;
  const modes = Array.isArray(item.modes)
    ? item.modes.flatMap((m) => { const s = record(m); return s ? [sanitizeVariableMode(s)].filter(Boolean) as VariableMode[] : []; })
    : [];
  if (!modes.length) modes.push({ id: `mode_${Date.now()}`, name: "Default", values: {} });
  const defaultModeId = typeof item.defaultModeId === "string" && modes.some((m) => m.id === item.defaultModeId)
    ? item.defaultModeId : modes[0].id;
  // Prune mode values that reference non-existent variables
  for (const mode of modes) {
    for (const varId of Object.keys(mode.values)) {
      if (!validVariableIds.has(varId)) delete mode.values[varId];
    }
  }
  return { id: item.id, name: item.name, modes, defaultModeId };
}

function sanitizeVariable(item: Record<string, unknown>): Variable | null {
  if (typeof item.id !== "string" || typeof item.name !== "string") return null;
  if (typeof item.collectionId !== "string") return null;
  if (typeof item.type !== "string" || !variableTypes.has(item.type as VariableType)) return null;
  return { id: item.id, name: item.name, collectionId: item.collectionId, type: item.type as VariableType, description: typeof item.description === "string" ? item.description : undefined };
}

function sanitizeComponentDefinitions(value: unknown): Record<string, ComponentDefinition> {
  const source = record(value) ?? {};
  ...
  for (const [id, def] of Object.entries(source)) {
    ...
    const properties = Array.isArray(d.properties) ? d.properties.flatMap((p) => {
      const prop = record(p);
      if (!prop || typeof prop.id !== "string" || typeof prop.name !== "string") return [];
      return [{ ...prop, id: prop.id, name: prop.name, type: prop.type === "boolean" || prop.type === "text" || prop.type === "instance-swap" ? prop.type : "boolean", defaultValue: prop.defaultValue ?? (prop.type === "boolean" ? false : "") }];
    }) : [];
    result[id] = { ...d, id, name: d.name, sourceNodeId: typeof d.sourceNodeId === "string" ? d.sourceNodeId : "", properties } as ComponentDefinition;
  }
  return result;
}
...
export function sanitizeDocument(value: unknown): { document: PageDocument; recovered: boolean } {
  ...
  const document = {
    ...
    colorStyles: sanitizeArray(source.colorStyles, sanitizeColorStyle),
    textStyles: sanitizeArray(source.textStyles, sanitizeTextStyle),
    effectStyles: sanitizeArray(source.effectStyles, sanitizeEffectStyle),
    variables: sanitizeArray(source.variables, sanitizeVariable),
    variableCollections: (() => {
      const variables = sanitizeArray(source.variables, sanitizeVariable);
      const validVariableIds = new Set(variables.map((v) => v.id));
      return sanitizeArray(source.variableCollections, (item) => sanitizeVariableCollection(item, validVariableIds));
    })(),
    components: sanitizeComponentDefinitions(source.components),
  } as PageDocument;
