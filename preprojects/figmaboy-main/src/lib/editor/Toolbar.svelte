<script lang="ts">
  ...
  // Close the shape menu when clicking outside the toolbar.
  $effect(() => {
    if (!shapeMenu) return;
    function handleClick(event: MouseEvent) {
      if (toolbarWrap && !toolbarWrap.contains(event.target as Node)) shapeMenu = false;
    }
...
<div class="toolbar-wrap" bind:this={toolbarWrap}>
  {#if shapeMenu}
