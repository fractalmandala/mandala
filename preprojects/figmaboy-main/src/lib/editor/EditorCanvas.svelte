<script lang="ts">
  ...
  import Rulers from "$lib/editor/Rulers.svelte";
  ...
  import { resolveConstraints } from "$lib/editor/constraints";
  ...
  const editingFontSize = $derived.by(() => {
    if (!editingNode || editingNode.type !== "text") return 0;
    const varId = editingNode.boundVariables?.fontSize;
    if (varId) { const resolved = session.allVariableValues.get(varId); if (typeof resolved === "number") return resolved; }
    return editingNode.fontSize;
  });
  const editingFillColor = $derived.by(() => {
    if (!editingNode || editingNode.type !== "text") return "#18181b";
    const varId = editingNode.boundVariables?.fill;
    if (varId) { const resolved = session.allVariableValues.get(varId); if (typeof resolved === "string") return resolved; }
    return editingNode.fill?.type === "solid" ? editingNode.fill.color : "#18181b";
  });
  ...
  $effect(() => {
    ...
    // Commit any in-progress text edits BEFORE cancelling the interaction;
    // cancelInteraction() reverts the gesture snapshot which would discard
    // the text changes the user typed.
  ...
  function resetInteractionState() {
    ...
    mode = "idle";
    // NOTE: releasePointer() is called explicitly by end() and cancelInteraction()
    // after the gesture is committed/cancelled so that persistencePaused stays
    // true through the entire document-mutation sequence.
  }

  function cancelInteraction() {
    // Commit any in-progress text edits so the user's typing is preserved
    // rather than reverted by the gesture cancel below.
    ...
    // Release after the gesture is settled so persistencePaused stays true
    // through the cancel sequence.
    releasePointer();
  }
  ...
  function resizeSelection(world: Point, constrain: boolean, fromCenter: boolean) {
    ...
    // Apply constraints/auto-layout to children of resized frames
    for (const [id, original] of startNodes) {
      const node = session.document.nodes[id];
      if (!node) continue;
      if ((node.type === "frame" || node.type === "group") && (node as never as { childIds: string[] }).childIds.length > 0) {
        if ((node as never as { layoutMode: string }).layoutMode && (node as never as { layoutMode: string }).layoutMode !== "none") {
          applyAutoLayout(node as never, session.document);
        } else {
          const oldParentSize = { width: original.width, height: original.height };
          for (const childId of (node as never as { childIds: string[] }).childIds) {
            const child = session.document.nodes[childId];
            if (!child?.constraints) continue;
            const oldChildBounds = { x: child.x, y: child.y, width: child.width, height: child.height };
            const resolved = resolveConstraints(child, { width: node.width, height: node.height }, child.constraints, oldParentSize, oldChildBounds);
            child.x = resolved.x;
            child.y = resolved.y;
  ...
  function resizeSingleSelection(world: Point, constrain: boolean, fromCenter: boolean) {
    ...
    // Apply constraints or auto-layout to children when a frame is resized
    if ((node.type === "frame" || node.type === "group") && (node as never as { childIds: string[] }).childIds.length > 0) {
      if ((node as never as { layoutMode: string }).layoutMode && (node as never as { layoutMode: string }).layoutMode !== "none") {
        applyAutoLayout(node as never, session.document);
      } else {
        ...
        for (const childId of (node as never as { childIds: string[] }).childIds) {
          ...
          if (!child?.constraints) continue;
          const oldChildBounds = { x: child.x, y: child.y, width: child.width, height: child.height };
          const resolved = resolveConstraints(child, { width, height }, child.constraints, oldParentSize, oldChildBounds);
          child.x = resolved.x;
          child.y = resolved.y;
          child.width = resolved.width;
          child.height = resolved.height;
  ...
  function end(event: PointerEvent) {
    ...
    if (mode === "edit-text" && pendingTextEditId) {
      ...
      if (mode === "move" && !dragged) {
        ...
        // Only fall back to the current parent when the drop point isn't
        // inside another frame AND the moved selection still overlaps its
        // original parent. Previously this also overrode an explicit target
        // frame, which prevented reparenting whenever frames overlapped.
        if (!targetFrameId && movedBounds) {
          ...
          if (currentParent?.type === "frame" && intersects(movedBounds, worldBounds(session.document, currentParent))) {
    ...
    resetInteractionState();
    // Release after all gesture/document mutations so persistencePaused
    // stays true through the commit sequence (prevents autosave races).
    releasePointer();
  }
  ...
  function windowBlur() {
    spacePressed = false;
    // cancelInteraction() now commits text edits internally, so a single
    // call covers both active pointer gestures and text editing.
    if (mode !== "idle") cancelInteraction();
  ...
  function startGuideDrag(event: PointerEvent, guide: { id: string; type: "h" | "v"; value: number }) {
    event.stopPropagation();
    event.preventDefault();
    session.rulerDrag = { id: guide.id, type: guide.type, isNew: false };
    ...
    const move = (e: PointerEvent) => {
      ...
      const val = guide.type === "h" ? worldY : worldX;
      session.updateGuide(guide.id, Math.round(val));
    };
    ...
    const up = (e: PointerEvent) => {
      ...
      const rect = host!.getBoundingClientRect();
      const RULER_SIZE = 20;
      const isOverRulerH = e.clientY - rect.top < RULER_SIZE;
      const isOverRulerV = e.clientX - rect.left < RULER_SIZE;
      if ((guide.type === "h" && isOverRulerH) || (guide.type === "v" && isOverRulerV)) {
        session.deleteGuide(guide.id);
      }
...
<div id="design-canvas" role="application" aria-label="Design canvas" data-mode={mode} data-gesture={session.hasActiveGesture ? "active" : "none"} class:grabbing={mode === "pan"} class:grab={session.activeTool === "hand" || spacePressed} class:crosshair={!['select', 'hand'].includes(session.activeTool)} class="canvas-host" bind:this={host} onpointerdown={begin} onwheel={wheel} oncontextmenu={contextMenu} onselectstart={preventNativeCanvasGesture} ondragstart={preventNativeCanvasGesture} onlostpointercapture={(event) => event.pointerId === activePointerId && mode !== "idle" && queueMicrotask(cancelInteraction)}>
  ...
  <svg bind:this={viewportElement} class="viewport-layer" width="100%" height="100%" aria-label="Design canvas" style:transform={viewportTransform(viewport)}>
    <g class="world">
      {#each session.document.rootIds as id}
        {#if session.document.nodes[id]}
          <CanvasNode node={session.document.nodes[id]} document={session.document} selectedIds={session.selectedIds} imageSources={session.imageSources} {unclippedFrameIds} activeModes={session.activeModes} variableValues={session.allVariableValues} onNodePointerDown={nodePointerDown} onNodeDoubleClick={nodeDoubleClick} onNodeContextMenu={nodeContextMenu} />
        {/if}
      {/each}
    </g>
    ...
    <g class="world-overlay">
      ...
      {#each session.userGuides as guide}
        {#if guide.type === "v"}<line class="user-guide" x1={guide.value} y1={-100000} x2={guide.value} y2={100000} stroke-width={1 / viewport.zoom} onpointerdown={(event) => startGuideDrag(event, guide)} />
        {:else}<line class="user-guide" x1={-100000} y1={guide.value} x2={100000} y2={guide.value} stroke-width={1 / viewport.zoom} onpointerdown={(event) => startGuideDrag(event, guide)} />{/if}
      {/each}
  ...
  {#if editingNode?.type === "text" && editingBounds && editingView}
    <textarea
      ...
      style:font-size={`${editingFontSize * viewport.zoom}px`}
...
<style>
  ...
  svg { display: block; }.canvas-background,.viewport-layer { position: absolute; inset: 0; }.viewport-layer { overflow: visible; transform-origin: 0 0; will-change: transform; }.guide,.marquee { pointer-events: none; }.guide { stroke: #ff4ecd; vector-effect: non-scaling-stroke; }.user-guide { stroke: #ff6b35; vector-effect: non-scaling-stroke; cursor: pointer; pointer-events: stroke; }.selection-ui { pointer-events: none; user-select: none; -webkit-user-select: none; }.selection-ui .handle, .selection-ui .rotate-handle, .selection-ui .line-handle { pointer-events: all; }.handle-nw,.handle-se { cursor: nwse-resize; }.handle-ne,.handle-sw { cursor: nesw-resize; }.handle-n,.handle-s { cursor: ns-resize; }.handle-e,.handle-w { cursor: ew-resize; }.rotate-handle { cursor: grab; }.line-handle { cursor: crosshair; }
