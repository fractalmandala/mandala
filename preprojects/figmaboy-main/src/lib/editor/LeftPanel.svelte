<script lang="ts">
  ...
  import StyleManager from "$lib/editor/StyleManager.svelte";
  import ComponentEditor from "$lib/editor/ComponentEditor.svelte";
  import VariableManager from "$lib/editor/VariableManager.svelte";
  ...
  function dropNode(draggedId: string, targetId: string) {
    ...
    // Non-container targets: insert before the target among its siblings.
    // Guard against a parent that is itself a non-container (shouldn't happen
    // in a well-formed document, but prevents a crash on undefined childIds).
    const parent = target.parentId ? session.document.nodes[target.parentId] : null;
    const siblings = parent && (parent.type === "frame" || parent.type === "group")
      ? parent.childIds
...
<aside class="left-shell">
  ...
  <section class="panel">
    {#if session.leftTab === "file"}
      ...
      <StyleManager {session} />
      <ComponentEditor {session} />
      <VariableManager {session} />
    {:else}
...
<style>
  .left-shell { position: absolute; inset: 0 auto 0 0; width: var(--left-panel-width, 297px); z-index: 30; display: grid; grid-template-columns: 56px 1fr; background: #292929; border-right: 1px solid #444; }.rail { border-right: 1px solid #3c3c3c; display: flex; flex-direction: column; align-items: center; padding-top: 8px; gap: 8px; }
