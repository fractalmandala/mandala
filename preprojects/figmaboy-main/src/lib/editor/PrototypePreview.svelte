  ...
  <main>
    {#if frame?.type === "frame"}
      <svg viewBox={`${frame.x} ${frame.y} ${Math.max(1, frame.width)} ${Math.max(1, frame.height)}`} preserveAspectRatio="xMidYMid meet">
        <CanvasNode node={frame} document={session.document} selectedIds={[]} imageSources={session.imageSources} interactive={false} preview={true} activeModes={session.activeModes} variableValues={session.allVariableValues} onPrototypeClick={activate} />
      </svg>
