...
export type LayoutAlign = "min" | "center" | "max" | "space-between" | "stretch";
...
export interface ComponentProperty {
  id: Id;
  name: string;
  type: "boolean" | "text" | "instance-swap";
...
export interface CommonNode {
  ...
  isInstance?: boolean;
  // Variable bindings: property path → variable id
  boundVariables?: Record<string, Id>;
}
...
export function defaultNode(type: NodeType, x: number, y: number, overrides: Partial<DesignNode> = {}): DesignNode {
  ...
  if (type === "frame" || type === "group") {
    return {
      ...base, type, childIds: [], clipContent: type === "frame",
      layoutMode: "none",
