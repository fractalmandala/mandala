...
Copy `codegraph.svelte` to `<name>graph.svelte` and replace only the `DEFAULT_DATA` block (starts at `const DEFAULT_DATA = `, ends at the closing `};`) with the new JSON. Copy `codegraph.sass` to `<name>graph.sass` verbatim — all five module graphs share this stylesheet. Do not rename the `.ahs` classes; the sass and markup must stay identical across module graphs.
...
1. **Register the sass**: add `@use '../modules/dev/styles/<name>graph'` to `index.sass` after the existing graph lines (currently lines 46–54).
