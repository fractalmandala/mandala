...
3. **Section 3 rule 1** (line ~60, "Never open a file in `docs/adr/`, `docs/design/`,
   or `docs/routing/` speculatively") → same rule over
   `adr/design/areas/guides/plans/archive`.
...
5. Repo-wide straggler grep:
   `grep -rn "docs/routing" src docs agents AGENTS.md CLAUDE.md` → every hit
   updated (ADR `relates_to` frontmatter referencing routing paths gets rewritten
   to the owning area doc) or justified in the report.
