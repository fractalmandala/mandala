...
2. **A2. Board.** Build `Canvas.svelte` + `_canvas.sass` (§4, §5): dotted-grid background,
   ...
   trackpad two-finger), zoom (ctrl/cmd+wheel toward cursor, clamp 0.4–2.0),
   `{#each canvas.tiles as tile (tile.id)}` → `<Tile {tile} />`. Stub
   `<Minimap />` / `<TileDock />`
...
3. **A3. Swap the shell.** Edit `+page.svelte` per §1: replace `workspace-wrapper` and the
   three `*Collapsed` branches with
   `<main class="board-region box w100 h100 overflow-hidden"><Canvas /></main>`.
   Delete `leftSidebarCollapsed` / `browserCollapsed` /
