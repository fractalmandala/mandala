...
- `canvas.svelte.ts` — tile positions, viewport coordinates, zoom, focus mode, template gallery visibility.
- `ide.svelte.ts` — active file, open tabs, file tree data, console logs, workspace configurations, chat messages, model registry, undo/redo stacks.
