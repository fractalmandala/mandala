...
**Related:** spec, ADR-008 (workspace serialization)
...
Vaults are a **Notes-template-only** concept that lives in `ide.svelte.ts` as `currentVaultName`, `currentVaultRoots`, `currentVaultTrees`, `savedVaults`, and `vaultError` `$state` fields.
...
Vault operations reuse the existing single-gateway pattern (ADR-004, `ipc.ts`). Folder picker reuses `selectDownloadDirectory`. Directory listing reuses `listDirectory`. File read/write reuse `readFile` / `writeFile`. **No new Tauri commands are added by this ADR** (the `set_active_template_menu` command for Window menu checkmark sync is separate and tracked under the menu-reorganization work).
