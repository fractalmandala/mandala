...
description: A header Navigation Menu component integrated with **SvelteKit** `page.url.pathname` store for automatic active link highlighting and interactive mega-menu hover delays.
---
      ...
      >
        {#if item.links}
          <button
            ...
            class:nav-menu__trigger--active={item.links.some(l => isActive(l.href))}
          >
            {item.title} &#9662;
          </button>
                ...
                {#each item.links as link}
                  <li>
                    <a
                      ...
                      class:nav-menu__link--active={isActive(link.href)}
        ...
        {:else}
          <a
            href={item.href}
            class="[ nav-menu__trigger ] [ pad8 padleft12 padright12 text-sm bold ]"
            class:nav-menu__trigger--active={isActive(item.href)}
