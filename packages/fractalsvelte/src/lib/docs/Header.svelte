...
<header class="app-header row ycenter gap32">
	...
	<nav class="row ycenter gap32">
		<a
			...
			href="/components"
			class:active={path.startsWith('/components')}
		>
			Components
		</a>
		<a class="header-link text-sm" href="/ai" class:active={path.startsWith('/ai')}>
			AI Elements
		</a>
		<a class="header-link text-sm" href="/blocks" class:active={path.startsWith('/blocks')}>
			Blocks
		</a>
		<ThemeSwitcher />
		<DocSearch />
