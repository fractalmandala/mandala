...
const BP_SUFFIX_RE = /^(.+)-(xs|sm|bs|lg|xl)$/;
...
export function generateCss({ classTokens, pxVars }: ScanResult): string {
	...
	for (const token of classTokens) {
		...
		if (bpMatch) {
			...
			const decls = resolveDeclarations(base);
			if (decls) mediaRules[bp as Breakpoint].push({ selector: token, decls });
			continue;
		}

		const decls = resolveDeclarations(token);
	...
	if (pxVars.size > 0) {
		...
		const lines = sorted.map((n) => `\t--px${n}: ${n}px;`).join('\n');
		parts.push(`:root {\n${lines}\n}`);
	}
