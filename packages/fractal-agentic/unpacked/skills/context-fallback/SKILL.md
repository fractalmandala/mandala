...
description: This skill should be used for diagnosing and mitigating context fallback: lost-in-middle failures, context poisoning, context clash, context confusion, attention-pattern issues, and agent performance fallback caused by accumulated or conflicting context.
---

# Context Fallback Patterns

Diagnose and fix context failures before they cascade. Context fallback is not binary — it is a continuum that manifests through five distinct, predictable patterns: lost-in-middle, poisoning, distraction, confusion, and clash. Each pattern has specific detection signals and mitigation strategies. Treat fallback as an engineering problem with measurable thresholds, not an unpredictable failure mode.

## When to Activate

Activate this skill when:
- Agent performance falls back unexpectedly during long conversations
...
Detect poisoning through these signals: fallback output quality on previously-successful tasks, tool misalignment (wrong tools or parameters), and hallucinations that persist despite explicit correction. When these cluster, suspect poisoning rather than model capability issues.
...
Curate what enters context rather than relying on models to ignore irrelevant content. Research shows even a single distractor document triggers measurable performance fallback — the effect follows a step function, not a linear curve. Multiple distractors compound the problem.
...
- **Models with very large context windows (1M+)** handle more raw context but still follow U-shaped fallback curves — bigger windows do not eliminate the problem, they delay it

Always benchmark fallback thresholds with your specific workload rather than relying on published benchmarks. Model-specific thresholds go stale with each model update (see Gotcha 2).

### Counterintuitive Findings

Account for these research-backed surprises when designing context strategies:

**Shuffled context can outperform coherent context.** Studies found incoherent (shuffled) haystacks can outperform logically ordered ones for some retrieval tasks (claim-context-fallback-distractor-shuffled). Coherent context may create false associations that confuse retrieval; incoherent context can force exact matching. Do not assume that better-organized context always yields better results — test both arrangements.

**Single distractors have outsized impact.** The performance hit from one irrelevant document is disproportionately large compared to adding more distractors after the first. Treat distractor prevention as binary: either keep context clean or accept significant fallback.

**Low needle-question similarity accelerates fallback.** Tasks requiring inference across dissimilar content fall back faster with context length than tasks with high surface-level similarity. Design retrieval to maximize semantic overlap between queries and retrieved content.

### When Larger Contexts Hurt

Do not assume larger context windows improve performance. Performance remains stable up to a model-specific threshold, then falls back rapidly — the curve is non-linear with a cliff edge, not a gentle slope. For many models, meaningful fallback begins at 8K-16K tokens even when windows support much larger sizes.
...
Recognize the cognitive bottleneck: even with infinite context, asking a single model to maintain quality across dozens of independent tasks creates fallback that more context cannot solve. Split tasks across sub-agents instead of expanding context.

## Practical Guidance

### The Four-Bucket Mitigation Framework

Apply these four strategies based on which fallback pattern is active:
...
**Isolate** — Split context across sub-agents or sessions to prevent any single context from growing past its fallback threshold. Use when confusion or clash symptoms appear, or when tasks are independent. This is the most aggressive strategy but often the most effective for complex multi-task systems.

### Architectural Patterns for Resilience

Implement just-in-time context loading: retrieve information only when the current reasoning step needs it, not preemptively. Use observation masking to replace verbose tool outputs with compact references after processing. Deploy sub-agent architectures where each agent holds only task-relevant context. Trigger compaction before context exceeds the model-specific fallback onset threshold — not after symptoms appear.

## Examples

**Example 1: Detecting Fallback**
```yaml
# Context grows during long conversation
turn_1: 1000 tokens
turn_5: 8000 tokens
turn_10: 25000 tokens
turn_20: 60000 tokens (fallback begins)
turn_30: 90000 tokens (significant fallback)
```
...
3. Implement compaction triggers before fallback becomes severe
...
7. Design for graceful fallback rather than assuming perfect conditions
8. Test with progressively larger contexts to find fallback thresholds

## Gotchas

1. **Normal variance looks like fallback**: Model output quality fluctuates naturally across runs. Do not diagnose fallback from a single drop in quality — establish a baseline over multiple runs and look for sustained, correlated decline tied to context growth. A 5-10% quality dip on one run is noise; the same dip consistently appearing after 40K tokens is signal.

2. **Model-specific thresholds go stale**: The fallback onset values in benchmark tables reflect specific model versions. Provider updates, fine-tuning changes, and infrastructure shifts can move thresholds by 20-50% in either direction. Re-benchmark quarterly and after any major model update rather than treating published thresholds as permanent.
...
5. **Prompt quality problems masquerade as fallback**: Poor prompt structure (ambiguous instructions, missing constraints, unclear task framing) produces symptoms identical to context fallback — inconsistent outputs, ignored instructions, wrong tool usage. Before diagnosing fallback, verify the same prompt works correctly at low context lengths. If it fails at 2K tokens, the problem is the prompt, not the context.

6. **Fallback is non-linear with a cliff edge**: Performance does not fall back gradually — it holds steady until a model-specific threshold, then drops sharply. Systems designed for "graceful fallback" often miss this pattern because monitoring checks assume linear decline. Set compaction triggers well before the cliff (at 70% of known onset), not at the onset itself.
...
- `evaluation`: fallback tests and production monitoring.

## References

Internal reference:
- [Fallback Patterns Reference](./references/patterns.md) - Read when: debugging a specific fallback pattern and needing implementation-level detection code (attention analysis, poisoning tracking, relevance scoring, recovery procedures)
...
- context-optimization - Read when: fallback is diagnosed and specific mitigation techniques (compaction, compression, masking) are needed
- evaluation - Read when: setting up production monitoring to detect fallback before it impacts users
