...
Run two independent reviewers (primary agent model + an external model) against the current task output. Both must return NICE before the code is pushed. If either returns NAUGHTY, fix all flagged issues, commit, and re-run fresh reviewers — up to 3 rounds.
...
#### Reviewer A: Primary agent reviewer (always runs)
...
#### Reviewer B: External Model (same-host fallback only if no external CLI installed)
...
**Agent CLI with sandbox flags** (if installed)
...
**Same-host agent fallback** (only if neither `codex` nor `gemini` is installed)
Launch a second agent reviewer (subagent_type: `code-reviewer`, model: `opus`). Log a warning that both reviewers share the same model family — true model diversity was not achieved but context isolation is still enforced.
...
Reviewer A (primary high-capability model):   [PASS/FAIL]
...
- Reviewer A (primary high-capability model) always runs — guarantees at least one strong reviewer regardless of tooling.
...
- External reviewers run with `--sandbox read-only` (when the host supports read-only sandbox) to prevent repo mutation during review.
