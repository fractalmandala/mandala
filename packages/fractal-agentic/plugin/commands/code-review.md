...
The PR Review Mode requires the `gh` CLI. If `gh` is not installed or not in PATH, fall back to local-only review:

1. Read the PR description and changed files from the user's context or from a local checkout of the branch.
2. Run the **Local Review Mode** phases (GATHER → REVIEW → REPORT) against the branch diff using `git diff`.
3. Skip Phases 7 (PUBLISH) and 8 (OUTPUT) for GitHub posting — report the review verbally or in the session instead.
4. Warn the user: `gh` is required for PR publishing and inline comments; local-only review is a degradation.

If the PR number or URL cannot be resolved without `gh`, stop with a clear error and the fallback instruction above.
