...
The CLI scans local agent transcript JSONL files under
...
without waiting for the current agent session to dequeue `/loop-status`. The
CLI writes:
...
timeout agent runtime tool calls.
