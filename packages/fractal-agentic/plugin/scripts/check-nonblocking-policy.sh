...
# Policy: docs/DEGRADATION.md — harness upgrades quality; never blocks product work.
...
# Runtime-binding files only (not DEGRADATION.md itself, which may quote anti-patterns).
files="
...
# DEGRADATION.md must exist and state the one-line principle
deg=$plugin_dir/docs/DEGRADATION.md
[ -f "$deg" ] || fail "missing $deg"
grep -Fq 'Harness upgrades quality' "$deg" || fail "DEGRADATION.md missing one-line principle"
pass "DEGRADATION.md present with principle"
