...
require_file "$plugin_dir/docs/DEGRADATION.md"
...
if command -v python3 >/dev/null 2>&1; then
  if python3 - "$orch/agents/openai.yaml" <<'PY'
import sys
from pathlib import Path
text = Path(sys.argv[1]).read_text(encoding="utf-8")
for key in ("display_name:", "short_description:", "default_prompt:"):
    if key not in text:
        print(f"FAIL: openai.yaml missing {key}", file=sys.stderr)
        sys.exit(1)
if "boss-orchestration" not in text and "Fractal Agentic" not in text:
