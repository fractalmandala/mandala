...
spawn discovery. Product work must not wait on that; see docs/DEGRADATION.md.
...
if [ "$check_only" -eq 1 ]; then
  ...
  printf '%s\n' "      Policy: docs/DEGRADATION.md — product work must never wait on pins."
  exit 0
fi
...
printf '%s\n' "  3. See plugin docs/DEGRADATION.md for the three-layer model."
printf '%s\n' ""
