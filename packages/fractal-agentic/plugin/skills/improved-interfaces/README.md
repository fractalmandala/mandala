...
Installs all seven skills together and updates in place. Run these inside the coding agent:
...
Works across coding-agent hosts. You can choose which skills to install or install all of them. `better-interface` coordinates the other six skills, so install the complete collection when you want holistic reviews.
...
In the coding agent, as a plugin. Plugin skills are namespaced, so every skill is prefixed with `interfaces:`.
...
In the coding agent, installed with the skills CLI:
...
On pin-capable / skill-native hosts:
