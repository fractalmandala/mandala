...
The `.claude-plugin/` directory packages the same `skills/` directory as a agent plugin named `interfaces`. It is the coding agent-specific and needs no change when adding a skill, but leave it in place.
