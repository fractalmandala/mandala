...
Continuous-learning-v2 stores observer data outside `~/.claude` so the agent host's sensitive-path guard does not block background instinct writes:
...
No extra `settings.json` hook block is required. the coding agent v2.1+ auto-loads the plugin `hooks/hooks.json`, and `observe.sh` is already registered there.
...
_Instinct-based learning: teaching the agent your patterns, one project at a time._
