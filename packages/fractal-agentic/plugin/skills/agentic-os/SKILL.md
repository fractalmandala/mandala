---
name: agentic-os
description: Build persistent multi-agent operating systems on the coding agent. Covers kernel architecture, specialist agents, slash commands, file-based memory, scheduled automation, and state management without external databases.
metadata:
  origin: ECC
---
...
Treat the coding agent as a persistent runtime / operating system rather than a chat session. This skill codifies the architecture used by production agentic setups: a kernel config that routes tasks to specialist agents, persistent file-based memory, scheduled automation, and a JSON/markdown data layer.

## When to Activate

- Building a multi-agent workflow inside the coding agent
- Setting up persistent the coding agent automation that survives session restarts
...
Each agent is a standalone markdown file in `agents/`. The agent loads the relevant agent file when routing a task.
...
For parallel execution, use the agent host's background task capability or shell scripts that invoke the coding agent with specific agent contexts.
...
Place command files in `.claude/commands/<command-name>.md`. the coding agent auto-discovers them. Users invoke them with `/<command-name>`.
...
Agentic OS tasks run on a schedule using external cron, not the agent host's built-in cron (which dies when the session ends).
...
Starting fresh every time the coding agent opens.
```
...
- [ ] Scheduled tasks use external cron (LaunchAgent, systemd, pm2), not the agent host's session cron
