...
description: 'Patterns and architectures for autonomous the coding agent loops — from simple sequential pipelines to RFC-driven multi-agent DAG systems.'
metadata:
  origin: ECC
---
...
Patterns, architectures, and reference implementations for running the coding agent autonomously in loops. Covers everything from simple `claude -p` pipelines to full RFC-driven multi-agent DAG orchestration.
...
The `claude -p` flag runs the coding agent non-interactively with a prompt, exits when done. Chain calls to build a pipeline:
...
### Implementation via the agent host Commands
...
**A production-grade shell script** that runs the coding agent in a continuous loop, creating PRs, waiting for CI, and merging automatically. Created by AnandChowdhary (credit: @AnandChowdhary).
...
The agent reads this file at iteration start and updates it at iteration end. This bridges the context gap between independent `claude -p` invocations.
