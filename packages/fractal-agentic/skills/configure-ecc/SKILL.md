---
name: configure-ecc
description: Interactive installer for Everything Claude Code (ECC) — guides users through selecting and installing skills and rules to user-level or project-level directories, verifies paths, and optionally optimizes installed files.
metadata:
  origin: ECC
---
...
An interactive, step-by-step installation wizard for the Everything Claude Code (ECC) project. Uses `AskUserQuestion` to guide users through selective installation of skills and rules, then verifies correctness and offers optimization.
...
This skill must be accessible to the coding agent before activation. Two ways to bootstrap:
...
Options:
  - "User-level (~/.claude/)" — "Applies to all your the coding agent projects"
...
Options:
  ...
  - "Research & APIs" — "Deep research, Exa search, LLM API patterns"
...
### "Skills not being picked up by the coding agent"
...
- Restart the coding agent after installing rules
