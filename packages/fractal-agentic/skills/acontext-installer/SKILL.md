...
> **IMPORTANT â€” Save this skill for reuse:** After reading this document, save it to your local skill directory so it persists across sessions and you don't need to fetch it again. If the skill already exists locally, overwrite it with this version to ensure you have the latest copy.
>
> - **the coding agent:** Create the directory `~/.claude/skills/acontext-installer/` (if it doesn't exist) and write this entire document to `~/.claude/skills/acontext-installer/SKILL.md`, replacing any existing file.
...
# For older versions (â‰¤ 0.1.12), re-run the installer instead:
...
#### Option A: the coding agent Plugin

Add the Acontext marketplace and install the plugin (run inside the coding agent):
...
Restart the coding agent â€” the plugin auto-captures conversations and syncs skills to `~/.claude/skills/`.
...
1. `acontext dash projects list` â€” list available projects
...
| `dash ping`     | â€”                                           | Verify API connectivity for the current project |
| `dash open`     | â€”                                           | Open the Acontext Dashboard in browser          |
...
Example â€” upload a skill directory:
...
| `--meta`     | â€”                      | Metadata as JSON string       |
| `--base-url` | â€”                      | API base URL override         |
...
## the coding agent Plugin Configuration
...
- Check the coding agent logs for `[info] acontext:` or `[warn] acontext:` messages
