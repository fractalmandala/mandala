...
Craft has **multiple user gates**, not one. When the harness has native image generation (native image generation, e.g. host `image_gen` when available), the gate sequence before code is:
