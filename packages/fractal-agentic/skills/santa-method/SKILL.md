...
```python
# Spawn reviewers in parallel (the coding agent subagents)
...
### Pattern A: the coding agent Subagents (Recommended)

Subagents provide true context isolation. Each reviewer is a separate process with no shared state.

```bash
# In a agent session, use the Agent tool to spawn reviewers
