...
description: Transform the coding agent into a fully autonomous agent system with persistent memory, scheduled operations, computer use, and task queuing. Replaces standalone agent frameworks (Hermes, AutoGPT) by leveraging the agent host's native crons, dispatch, MCP tools, and memory. Use when the user wants continuous autonomous operation, scheduled tasks, or a self-directing agent loop.
metadata:
  origin: ECC
---
...
Turn the coding agent into a persistent, self-directing agent system using only native features and MCP servers.
...
Use the agent host's built-in memory system enhanced with MCP memory server for structured data.
...
Use the agent host's scheduled tasks to create recurring agent operations.
...
Trigger the coding agent agents remotely for event-driven workflows.
...
# GitHub webhook → dispatch → coding agent → fix → PR
...
Leverage the agent's computer-use MCP for physical world interaction.
...
| ---------------- | --------------------------------- | --------------------------------------- |
| Gateway/Router   | the coding agent dispatch + crons      | Scheduled tasks trigger agent sessions  |
| Memory System    | host memory + MCP memory server | Built-in persistence + knowledge graph  |
