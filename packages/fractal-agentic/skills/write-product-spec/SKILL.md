...
Implementation details, validation, and test planning live in a companion `TECH.md`, produced by the `write-tech-spec` skill. Writing the product spec is usually the first step of a two-step process: once `PRODUCT.md` is agreed on, invoke `write-tech-spec` to produce `TECH.md` for the same features (or let the user know that's the expected next step). The product spec should be written so the tech spec can be written directly from it.

Write specs to `PRODUCT.md` in the root folder of product project.

## Before writing

Gather only the context you need: feature names, feature summaries, target users, key behaviors, edge cases, and how the features will be validated. Ask questions for missing contexts - don't guess and don't assume.
