...
FractalKnow is the new Tauri + SvelteKit target for the OpenKnowledge migration.
It is intentionally separate from the current React/Electron app while the port
is sliced across desktop bridge, shell, editor, and package-integration work.

Styling uses indented `.sass` files and `<style lang="sass">` blocks with tabs.
