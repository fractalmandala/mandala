...
Welcome, Agent! This document serves as the orientation and technical guide for all autonomous AI agents collaborating on the **FractalEngine Studio** codebase.

It is not allowed to work in this project if you ignore this file, and the documents it references.

## Fractal Agentic plugin mandate (required session bootstrap)

**Before any other work** on a non-trivial task, detect and use the monorepo **Fractal Agentic** plugin. Do not wait for the user to tag it.

| Item | Value |
|---|---|
| Expected root (from monorepo) | `../../agentic/fractal-agentic` (from this app) or `agentic/fractal-agentic` (from monorepo root) |
| Canonical snippet | [`agentic/fractal-agentic/project-integration/AGENTS-SNIPPET.md`](../../agentic/fractal-agentic/project-integration/AGENTS-SNIPPET.md) |
| Probe | `sh ../../agentic/fractal-agentic/scripts/resolve-plugin-root.sh` (from monorepo-aware cwd) |

**Accessible** = you can read that root’s `AGENTS.md` and `skills/boss-orchestration/SKILL.md`.

When found: load the plugin map + **boss-orchestration**; select a domain boss (usually **Svelte** / **Design** / **Creator** for this app); for any change set that claims completion, run the orchestration ship gate (`ship|fix-first|rethink`). Prefer `/orchestrate` and mapped skills over ad-hoc process.

When missing: state once and continue under **this** AGENTS.md only.

This file’s stack rules (runes, indented SASS, IPC, undo, docs) **override** generic plugin defaults when they conflict.
