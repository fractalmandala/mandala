...
/** One sub-path in a vector block. Closed paths render with Z and have their
 *  fill applied; open paths are stroke-only. */
...
/** Primitive shapes the draw tool can stamp onto the canvas. A subset of
 *  DesignBlock['type'] — 'card'/'frame'/'container'/'image' are placed via
 *  the dock/templates instead of drag-to-draw. */
export type ShapeType = 'rectangle' | 'ellipse' | 'line' | 'arrow' | 'polygon' | 'star' | 'vector';

/** A designer's note pinned to a block, queued for the AI agent to act on. */
export type UIAnnotation = {
	id: string;
	prompt: string;
	status: 'draft' | 'queued' | 'applied' | 'dismissed';
	createdAt: number;
};

/** A node in the design canvas scene graph — a frame, shape, text, image, or
 *  vector block. `x`/`y` are relative to `parentId` (or canvas-absolute when
 *  `parentId` is null); `children` lists child block ids in render order. */
export interface DesignBlock {
	id: string;
	type: 'text' | 'image' | 'card' | 'frame' | 'container' | ShapeType;
	name: string;
	x: number;
	y: number;
	w: number;
	...
	/** Overrides the HTML tag codegen emits for this block (default depends
	 *  on `type` — see `tagForBlock` in codegen.ts). */
	htmlTag?: string;
}

const BLOCK_TYPES: readonly DesignBlock['type'][] =
	['text', 'image', 'card', 'frame', 'container', 'rectangle', 'ellipse', 'line', 'arrow', 'polygon', 'star', 'vector'];

/** Runtime guard for narrowing a free-form string (e.g. from a `<select>`) into
 *  a valid `DesignBlock['type']` without an `any`/unchecked cast. */
export function isBlockType(value: string): value is DesignBlock['type'] {
	return (BLOCK_TYPES as readonly string[]).includes(value);
}

export const HTML_TAGS = ['div', 'section', 'article', 'header', 'footer', 'main', 'nav', 'aside',
	'h1', 'h2', 'h3', 'p', 'span', 'button', 'a', 'img', 'ul', 'li'] as const;
export type HtmlTag = (typeof HTML_TAGS)[number];

export function isHtmlTag(value: string): value is HtmlTag {
	return (HTML_TAGS as readonly string[]).includes(value);
}
