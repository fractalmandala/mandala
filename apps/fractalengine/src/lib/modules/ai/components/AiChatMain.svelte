<script lang="ts">
	...
	import { aiState as ideState } from "$lib/state/ai.svelte";
	import { tick } from "svelte";
	import { onAiUsage, readFile, type TokenUsage } from "$lib/ipc";
	import { errorMessage } from "$lib/errors";
	import { dictation } from '$lib/state/dictation.svelte';
	import { maxContextTokensFor } from "$lib/data/modelContextWindows";
	...
	import Actions from "$lib/components/ai-elements/Actions.svelte";
	import ModelSelector from "$lib/components/ai-elements/ModelSelector.svelte";
	import { Context } from "$lib/components/ai-elements/context";
	import {
		modelRegistry,
		skillsApi,
				...
				{#each ideState.chatMessages as msg (msg.id)}
					{#if msg.role === "user"}
						<div class="msgbox-user">
							<div class="msgbox-user-bubble">
								<Response content={msg.content} />
							</div>
							<div class="row gap8 ycenter xright">
								<span class="text-xs col3">{msg.timestamp}</span>
							</div>
						</div>
					{:else if msg.isError}
