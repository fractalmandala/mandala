...
# ADR-008: Workspace Serialization Format and Native Menu Bar Event Linkage
...
Tauri builds the native operating system menu in Rust ([`lib.rs`](file:///Users/amrit/fractals/apps/fractalengine/src-tauri/src/lib.rs)) with specific item IDs and keyboard shortcut mappings:
...
The root Svelte layout component ([`+layout.svelte`](file:///Users/amrit/fractals/apps/fractalengine/src/routes/+layout.svelte)) listens to this event on mount, mapping event payloads directly to Svelte state functions in `ideState`:
