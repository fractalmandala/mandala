...
# AI Layer Remediation — Two-Stream Fix Plan
...
- `sendAiMessage` sidecar branch: resolve `recommendedModels.find(m => m.id ===
  selectedModelId)?.localPath` in addition to the two custom ids; clear error message
  naming the Settings section when unresolved.
