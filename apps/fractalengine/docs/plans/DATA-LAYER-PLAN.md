...
# Data Layer (SQLite + FTS5) & Bookmarks Pilot — Two-Stream Execution Plan
...
Follow the file's existing per-function pattern (`isTauri() ? invoke('...') :
mockIpc....`). Append exactly:
...
Same signatures, minimal bodies (`return []` / `throw new Error('mock: not yet
implemented')` for mutations), each tagged `// Stream B replaces (data-layer plan)`.
