...
Every selector in all nine stylesheets carries its scoping root class, so aggregation through `index.sass` cannot leak into app chrome. Registration lives at [index.sass](file:///Users/amrit/fractals/apps/fractalengine/src/lib/styles/index.sass) lines 46–54, one `@use` per report.

## Module-flow family — the `.ahs` registry

The five module graphs (Code, Agent, Design, Media, Notes) share one stylesheet verbatim: [codegraph.sass](file:///Users/amrit/fractals/apps/fractalengine/src/lib/modules/dev/styles/codegraph.sass), copied per report so each pair stays self-contained (see [ADR-042](../adr/ADR-042-dev-graph-gallery-and-report-splitting.md)). It was originally authored as [apphealth.sass](file:///Users/amrit/fractals/apps/fractalengine/src/lib/components/apphealth/apphealth.sass) (539 lines) for the `AppHealthScan` renderer.
...
Node group and edge colors live in [graphLayout.ts](file:///Users/amrit/fractals/apps/fractalengine/src/lib/components/apphealth/graphLayout.ts) so the SVG renderer can read them; SASS only consumes them via inline styles.

Group colors — `GROUP_COLORS`, [graphLayout.ts:18](file:///Users/amrit/fractals/apps/fractalengine/src/lib/components/apphealth/graphLayout.ts):
...
Edge colors — `EDGE_COLORS`, [graphLayout.ts:41](file:///Users/amrit/fractals/apps/fractalengine/src/lib/components/apphealth/graphLayout.ts):
...
`ahs-dash` ([codegraph.sass:245](file:///Users/amrit/fractals/apps/fractalengine/src/lib/modules/dev/styles/codegraph.sass)) — marching dashes on edges during flow playback.
...
|---|---|---|
| [healthgraph.sass](file:///Users/amrit/fractals/apps/fractalengine/src/lib/modules/dev/styles/healthgraph.sass) | `.healthgraph` | `tile-in` (:317), `flash` (:348), `pulse` (:463) |
| [flowgraph.sass](file:///Users/amrit/fractals/apps/fractalengine/src/lib/modules/dev/styles/flowgraph.sass) | `.flowgraph` | `node-in` (:185), `spin-ring` (:280), `fade-in` (:290), `flow` (:330), `pulse` (:478) |
| [atlasgraph.sass](file:///Users/amrit/fractals/apps/fractalengine/src/lib/modules/dev/styles/atlasgraph.sass) | `.atlasgraph` | `atlas-spin` (:278) |
| [wikigraph.sass](file:///Users/amrit/fractals/apps/fractalengine/src/lib/modules/dev/styles/wikigraph.sass) | `.wikigraph` | `wiki-spin` (:220) |
