---
id: sp-plan-2026-07-15-shared-memory-mve-two-stream-implementation
title: Superpowers Plan: 2026-07-15-shared-memory-mve-two-stream-implementation
type: archive
...
The operator must complete **Phase 0** once before launching either role. After Phase 0, the operator's only agent instructions are:
...
The agents must not edit the same files or inspect each other's experimental material before both handoffs are complete. A later integration pass combines their work, runs the paired experiment in disposable worktrees, and applies the go/no-go rules.
...
This operator-only phase is mandatory. Agent A and Agent B read it for context but do not execute it. It creates one coordination branch, one clean experiment fixture, and two isolated implementation worktrees. It also records the fixture hash in the repository so neither agent needs to receive it separately.
...
### 0.4 Create the two implementation worktrees

Both worktrees branch from the same final coordination commit, which contains this manual and the recorded fixture hash.
...
Open one agent task in each corresponding worktree. Do not open both tasks against the coordination worktree or the original monorepo worktree.
...
Both statuses must be clean. Only then give the two one-line role assignments.

### 0.5 Setup failure conditions

Do not launch either agent if any of these is true:
...
5. Confirm that you are working on an isolated branch or worktree. If isolation is not available, stay strictly within your owned files and report the limitation; do not create edits that could overwrite concurrent work.
6. Do not open, search, diff, or inspect the other agent's branch, worktree, owned files, messages, or generated artifacts.
...
**Branch:** `memory-mve-evaluation`

**Required worktree:** `fractals-memory-mve-agent-a`
...
1. Confirm `git branch --show-current` returns `memory-mve-evaluation` and `git status --short` is clean. Otherwise report `setup-incomplete` and stop.
...
4. Confirm Agent B's worktree/branch is not opened or searched.
...
Inspect the final diff and confirm no Agent B-owned file changed.
...
The final status must be clean. Do not stage or commit a reserved integration file.
...
**Branch:** `memory-mve-projection`

**Required worktree:** `fractals-memory-mve-agent-b`
...
1. Confirm `git branch --show-current` returns `memory-mve-projection` and `git status --short` is clean. Otherwise report `setup-incomplete` and stop.
...
5. Confirm Agent A's branch/worktree and evaluation files will not be opened or searched.
...
Inspect the final diff and confirm no Agent A-owned file changed.
...
The final status must be clean. `agents/memory-mve/BASE_COMMIT` must remain unchanged.
...
The agents start from the two Phase 0 worktrees. Each reads the same recorded `BASE_COMMIT` and proceeds only within its own files.
...
Create the integration branch from the untouched coordination branch, not from either implementation branch:
...
3. Review commits individually before merging/cherry-picking.
...
### I4. Merge the implementation branches
...
Any merge conflict means file ownership was violated or the coordination base drifted. Investigate the branch histories; do not force a resolution merely to continue.

After the merge, create the local `tests/memory-mve/results/memory-map.json` by mapping Agent A's frozen semantic concepts to Agent B's frozen `MVE-*` IDs. Do not edit prompts, rubrics, projection items, or expected outcomes. Record missing mappings as coverage gaps.
