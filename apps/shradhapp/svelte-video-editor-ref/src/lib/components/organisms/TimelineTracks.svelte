<script lang="ts">
	...
	function onDropBinItem(trackId: string, e: DragEvent) {
		...
		const customItemId = type ? e.dataTransfer?.getData(type) : null;
		const itemId =
			customItemId ||
...
<div class="flex min-h-0 flex-1 flex-col bg-muted/15">
	<div
		bind:this={scrollEl}
		bind:clientWidth={containerW}
		id="timeline-scroll"
		class="relative min-h-0 flex-1 overflow-auto bg-background"
		onwheel={onWheel}
		onscroll={onScroll}
	...
	<div class="flex items-center gap-3 border-t border-border/60 bg-muted/25 px-2 py-1">
		...
		<div class="flex shrink-0 items-center gap-1">
			{#if isMobile}
				...
				{/if}
				<div class="mx-0.5 h-5 w-px bg-border/70"></div>
			{/if}
