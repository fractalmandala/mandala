<script lang="ts">
	...
	function addToTimeline(item: BinItem) {
		const track = editor.project.tracks.find((track) => !track.locked);
		if (!track) return;
		editor.addClipFromBin(item, track.id, editor.playheadF, false);
	}
</script>
