<script lang="ts">
	...
	const editor = new TimelineEditorStore(migrateProject(initialProject()), {
		emitChange: (nextProject) => onChange(nextProject),
	...
	function optionalGenerateWaveform(): EditorHost['generateWaveform'] {
		return generateWaveform ? (assetId) => generateWaveform?.(assetId) ?? Promise.reject() : undefined;
	}

	setTimelineEditor(editor);
	const host = setEditorHost({
		resolveAsset: (...args) => resolveAsset(...args),
		generateThumbnail: (...args) => generateThumbnail(...args),
		generateWaveform: optionalGenerateWaveform(),
		onExport: (...args) => onExport(...args),
		can: (...args) => can(...args)
	});
	...
	let timelineH = $state<number | null>(initialTimelineHeight());
	...
	function initialBinWidth() {
		return binWidth ?? null;
	}

	function initialInspectorWidth() {
		return inspectorWidth ?? null;
	}

	let containerW = $state(0);
	let binW = $state<number | null>(initialBinWidth());
	let inspectorW = $state<number | null>(initialInspectorWidth());
	...
	function hasBinDragType(e: DragEvent): boolean {
		const types = e.dataTransfer?.types ?? [];
		return types.some((type) => type.startsWith(BIN_ITEM_MIME_PREFIX) || type === BIN_ITEM_TEXT_MIME);
	}

	function droppedBinItemId(e: DragEvent): string | null {
		const customType = e.dataTransfer?.types.find((type) => type.startsWith(BIN_ITEM_MIME_PREFIX));
		const customItemId = customType ? e.dataTransfer?.getData(customType) : null;
		return (
			customItemId ||
			binItemIdFromTextPayload(e.dataTransfer?.getData(BIN_ITEM_TEXT_MIME) ?? '')
		);
	}
	...
	function onPreviewDragOver(e: DragEvent) {
		if (!hasBinDragType(e) || !firstUnlockedTrackId()) return;
		e.preventDefault();
		if (e.dataTransfer) e.dataTransfer.dropEffect = 'copy';
	}

	function onPreviewDrop(e: DragEvent) {
		const itemId = droppedBinItemId(e);
		const item = editor.project.bin.find((binItem) => binItem.id === itemId);
		const trackId = firstUnlockedTrackId();
		if (!item || !trackId) return;
		e.preventDefault();
		editor.addClipFromBin(item, trackId, editor.playheadF, e.metaKey || e.ctrlKey);
	}
</script>
	...
	<Tooltip text={handlers.label} side={handlers.tipSide}>
		{#snippet child({ props })}
			<button
				...
				class="group flex w-1.5 shrink-0 cursor-col-resize touch-none appearance-none items-center justify-center border-0 border-r border-border/50 bg-muted/30 p-0 transition-colors hover:bg-primary/10 focus-visible:bg-primary/10 focus-visible:outline-none"
			...
			>
				<div class="h-10 w-0.5 rounded-full bg-border transition-colors group-hover:bg-primary/70"></div>
			</button>
		{/snippet}
	</Tooltip>
{/snippet}
...
<div
	class="flex h-full min-h-0 flex-col overflow-hidden rounded-md border border-border/60 bg-background text-foreground"
...
>
	...
	<div class="flex min-h-0 flex-1 bg-muted/10" bind:clientWidth={containerW}>
		{#if !isMobile}
			...
			{:else}
				<aside class="shrink-0 border-r border-border/60 bg-muted/35" style="width: {binWpx}px;">
			...
			{/if}
		{/if}
		<main class="flex min-w-0 flex-1 flex-col bg-muted/10">
			{#if preview}
		...
		{#if !isMobile}
			...
			{:else if editor.activeClip}
				...
				<aside class="shrink-0 border-l border-border/60 bg-muted/35" style="width: {inspectorWpx}px;">
	...
	<Tooltip text={t.resizePanes}>
		{#snippet child({ props })}
			<button
				...
				class="group flex h-2 shrink-0 cursor-row-resize touch-none appearance-none items-center justify-center border-0 border-y border-border/60 bg-muted/30 p-0 transition-colors hover:bg-primary/10 focus-visible:bg-primary/10 focus-visible:outline-none"
			...
			>
				<div class="h-0.5 w-12 rounded-full bg-border transition-colors group-hover:bg-primary/70"></div>
			</button>
		{/snippet}
	</Tooltip>

	<div class="flex min-h-0 shrink-0 flex-col bg-muted/15" style="height: {timelineHpx}px;">
		{#if tracks}
