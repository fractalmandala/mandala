<script lang="ts">
	...
	function acceptsDrag(e: DragEvent): boolean {
		...
		return types.some((t) => t.startsWith(BIN_ITEM_MIME_PREFIX) || t === BIN_ITEM_TEXT_MIME);
	}
...
<div
	class={cn('relative flex w-full border-b border-border/60 bg-background', reordering && 'z-30')}
...
>
	<div
		role="presentation"
		class="sticky left-0 z-20 flex shrink-0 cursor-grab flex-col justify-center gap-0.5 overflow-hidden border-r border-border/60 bg-muted/30 px-2 active:cursor-grabbing"
	...
	<ContextMenu>
		{#snippet trigger({ props })}
			<div
				...
				class={cn(
					'relative h-full flex-1 bg-background transition-colors',
					dragOver ? 'bg-primary/10' : 'hover:bg-muted/15',
	...
	<div
		role="presentation"
		class={cn(
			'absolute right-0 bottom-0 left-0 z-20 h-1 cursor-row-resize hover:bg-primary/10',
