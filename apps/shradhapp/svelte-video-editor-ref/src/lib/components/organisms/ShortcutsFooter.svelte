...
<div
	class="flex items-center gap-3 overflow-x-auto border-t border-border/60 bg-muted/20 px-3 py-1 text-[10px] whitespace-nowrap text-muted-foreground"
>
	{#each hints as hint (hint.keys)}
		<span>
			<kbd class="rounded border border-border/60 bg-background px-1 font-sans">{hint.keys}</kbd>
			{hint.label}
		</span>
	{/each}
</div>
