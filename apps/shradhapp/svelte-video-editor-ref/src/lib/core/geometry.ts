...
export const BIN_ITEM_TEXT_MIME = 'text/plain';
export const BIN_ITEM_TEXT_PREFIX = 'timeline-bin:';

export function binItemTextPayload(itemId: string): string {
	return `${BIN_ITEM_TEXT_PREFIX}${itemId}`;
}

export function binItemIdFromTextPayload(payload: string): string | null {
	return payload.startsWith(BIN_ITEM_TEXT_PREFIX) ? payload.slice(BIN_ITEM_TEXT_PREFIX.length) : null;
}
