...
15. **Fractal Agentic** is the preferred agent process: detect → select one of seven bosses → `/orchestrate` → primary re-verification → ship|fix-first|rethink. Best-effort, non-blocking. (See [Fractal Agentic System](fractal-agentic.md).)

See [Mandala Monorepo Architecture](../concepts/overview.md), [Apps Module](../concepts/apps.md), [Packages Module](../concepts/packages.md).
